...
const float culling_distance = 50.0f; 
const float view_distance = 10000000.0f; 
...
void draw_face(face f) {
    ...
    vertex projected_vertices[16]; // Support up to 16 vertices per face
    bool all_in_front = true;
    float face_depth = 0.0f;
    
    for (int i = 0; i < f.vertex_count && i < 16; i++) {
        projected_vertices[i] = project_vertex(f.vertices[i], camera.x, camera.y, camera.z, 
                                             camera.yaw, camera.pitch, 90.0f, 
                                             (float)screen_width / (float)screen_height, 0.1f);
        if (projected_vertices[i].z <= 0.1f) {
            all_in_front = false;
        }
        face_depth += projected_vertices[i].z;
    }
    
    if (!all_in_front) return; // Skip if any vertex is behind camera
    
    face_depth /= f.vertex_count; // Average depth for the whole face
    
    // Simple face filling using scanline approach for quadrilaterals
    if (f.vertex_count == 4) {
        // Convert projected vertices to screen coordinates
        int screen_coords[4][2];
        for (int i = 0; i < 4; i++) {
            screen_coords[i][0] = (int)(projected_vertices[i].x + 0.5f);
            screen_coords[i][1] = (int)(projected_vertices[i].y + 0.5f);
        }
        
        // Find bounding box
        int min_x = screen_coords[0][0], max_x = screen_coords[0][0];
        int min_y = screen_coords[0][1], max_y = screen_coords[0][1];
        
        for (int i = 1; i < 4; i++) {
            if (screen_coords[i][0] < min_x) min_x = screen_coords[i][0];
            if (screen_coords[i][0] > max_x) max_x = screen_coords[i][0];
            if (screen_coords[i][1] < min_y) min_y = screen_coords[i][1];
            if (screen_coords[i][1] > max_y) max_y = screen_coords[i][1];
        }
        ...
        // Simple point-in-quad test and fill
        for (int y = min_y; y <= max_y; y++) {
            for (int x = min_x; x <= max_x; x++) {
                // Simple point-in-polygon test using cross product method
                bool inside = true;
                for (int i = 0; i < 4; i++) {
                    int j = (i + 1) % 4;
                    int x1 = screen_coords[i][0], y1 = screen_coords[i][1];
                    int x2 = screen_coords[j][0], y2 = screen_coords[j][1];
                    
                    // Cross product to determine which side of the edge the point is on
                    int cross = (x2 - x1) * (y - y1) - (y2 - y1) * (x - x1);
                    if (cross < 0) {
                        inside = false;
                        break;
                    }
                ...
                if (inside) {
                    set_pixel(x, y, f.ascii, f.color, face_depth);
                }
            }
        }
    } else {
        // For non-quad faces, fall back to drawing vertices and edges
        for (int i = 0; i < f.vertex_count; i++) {
            dot d;
...
void draw_test_objects() {
    ...
    dot dots[8] = {
        {{cube3_vertices[0]}, 'A', 91}, // Red
        {{cube3_vertices[1]}, 'B', 92}, // Green
        {{cube3_vertices[2]}, 'C', 94}, // Blue
        {{cube3_vertices[3]}, 'E', 93}, // Yellow
        {{cube3_vertices[4]}, 'F', 95}, // Magenta
        {{cube3_vertices[5]}, 'G', 96}, // Cyan
        {{cube3_vertices[6]}, 'H', 97}, // White
        {{cube3_vertices[7]}, 'I', 90}  // Dark gray
    };
...
void init_mouse_camera() {
    ...
    mouse_captured = true;  // Re-enabled mouse capture for camera control
}
